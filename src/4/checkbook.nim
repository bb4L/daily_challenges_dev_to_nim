...
proc checkbook*(data:string):string = 
    ...
    for line in mylines[1.. len(mylines)-1]:
        ...
        result_data.add(cleaned_line[0..1].join(" ") & " " & fmt("{expence:9.2f}").strip().strip(trailing=true) & " Balance " & $(val))

    result_data.add("Total expense " & fmt("{(init_balance-balance):9.2f}").strip().strip(trailing=true))
    result_data.add("Average expense " & fmt("{(init_balance-balance)/toFloat(exp_count):9.2f}").strip().strip(trailing=true))
